/// @description Insert description here
// You can write your code in this editor
event_inherited();


#region glitch effect
//shader_set(shdBktGlitch);
////resets the shader
//BktGlitch_config_zero();

//BktGlitch_set_jumbleness(random_range(0.70,0.75));
//BktGlitch_set_jumble_resolution(random_range(0.22,0.30));
//BktGlitch_set_jumble_shift(random_range(0.13,0.20));
//BktGlitch_set_channel_shift(0.44333);

//BktGlitch_set_intensity(1);

//// Inherit the parent event
//draw_sprite(sprite_index, 0 , real_round(x - 3 * dir), real_round(y));

//shader_reset();
//// Inherit the parent event
event_inherited();

#endregion

//draw_text(bbox_left, bbox_bottom + 3, "angle = " + string(bullet_angle))
//draw_text(bbox_left, bbox_bottom + 3, "x_spd = " + string(x_spd))
//draw_text(bbox_left, bbox_bottom + 10, "y_spd = " + string(y_spd))